...
int check_student_data() {
    ...
    while (fgets( buffer, 255 , datei) != NULL) {
        sscanf(buffer, "%49[^;];%49[^;];%49[^,\n],%49[^,\n],%49[^,\n]",
               snow[counter].ski_resort, snow[counter].surname, snow[counter].day[0].hobby,
               snow[counter].day[1].hobby, snow[counter].day[2].hobby);
        fflush(datei);
...
int main(int argc , char * argv[]) {
    ...
    for ( int i = 0; i < 30; i++) {
        ...
        for (int a = 0; a < 30; a++) {
            test_counter++;
            if((strcmp(snow[i].ski_resort, product[a].name ) == 0) && (strcmp(snow[i].surname, product[a].surname ) == 0) ) {
                test_counter = 0;
                //printf("%s, %s %i : \n", product[a].product, product[a].store, (product[a].price - product[a].weight)/60);
                for ( int b = 0; b < 3; b++) {
                    if (strcmp(snow[i].day[b].hobby, course) == 0 ) {
                        if ((((product[a].log_out_time - product[a].log_on_time) / 60) ) < presence_time*0.75){
                            printf("%s %s: %s,  %i Minuten \n", product[a].name, product[a].surname, snow[i].day[b].hobby, (product[a].log_out_time - product[a].log_on_time) / 60);
                        }else if ((((product[a].log_out_time - product[a].log_on_time) / 60) ) >= presence_time*0.75){
                            continue;
                        }else if((product[a].log_out_time - product[a].log_on_time) == 0) {
                            //printf("%s %s: nicht anwesend \n", product[a].name, product[a].surname);
                        }else {
                            //printf("%s %s: nicht anwesend \n", product[a].name, product[a].surname);
                        }
            ...
            printf("%s %s: nicht anwesend \n", snow[i].ski_resort, snow[i].surname);
        }
